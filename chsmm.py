...
if __name__ == "__main__":
    ...
    if len(args.load) > 0:
        ...
        saved_stuff = torch.load(args.load, torch.device('cpu')) 
    ...
    if args.optim == "adagrad":
        optalg = optim.Adagrad([p for p in net.parameters() if p.requires_grad], lr=args.lr)
        for group in optalg.param_groups:
            for p in group['params']:
                optalg.state[p]['sum'].fill_(0.1)
    elif args.optim == "rmsprop":
        optalg = optim.RMSprop([p for p in net.parameters() if p.requires_grad], lr=args.lr)
    elif args.optim == "adam":
        optalg = optim.Adam([p for p in net.parameters() if p.requires_grad], lr=args.lr)
    else:
        optalg = None
    ...
    def gen_from_src():
        ...
        if len(args.wid_workers) > 0:
            ...
            print("worker", wid, "doing lines", startln, "thru", endln-1, file=sys.stderr)
